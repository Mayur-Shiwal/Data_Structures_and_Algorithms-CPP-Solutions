...
private:
    ...
    int sumOfNodes(Node* root, int &nodesCount){
        if(!root) return 0;
        ...
        nodesCount++;
        return root->data + sumOfNodes(root->left, nodesCount) + sumOfNodes(root->right, nodesCount);
    }
    void solve(Node* root, int target){
        ...
        if(checkBST(root, INT_MIN, INT_MAX)){
            int nodesCount = 0;
            int s = sumOfNodes(root, nodesCount);
            if(s == target) mini = min(mini, nodesCount);
        }
